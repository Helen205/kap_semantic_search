...
def get_data_from_html(html_file):
    try:
...
def process_table_data(table_data, notification_id, table_count):
    try:
        if not table_data:
            return
...
def extract_table_data(html, notification_id):
    if not html:
    ...
    try:
        ...
        for table in tables:
            ...
            if 'financial-header-table' in table.get('class', []) and current_table_data:
                process_table_data(current_table_data, notification_id, table_count)
                table_count += 1
                current_table_data = []
...
def run_next_script():
    try:
        ...
        logger.error(f"Error running table_chunk.py: {e}")
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        if not html_files:
            logger.error("No HTML files to process.")
            exit(1)
        ...
        for html_file in html_files:
            try:
                ...
                html_content = get_data_from_html(html_file)
                if html_content:
                    extract_table_data(html_content, notification_id)
